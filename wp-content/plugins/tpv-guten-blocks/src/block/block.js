...
registerBlockType('tpv/guten-blocks-accordion-container', {
	...
	attributes: {
		accordionName: {
			type: 'string',
			source: 'attribute',
			default: parseInt( 100000 * Math.random() ) + '-acc-container',
		...
		return (
			<div id={accordionName} className={className}>
				...
				<MyBlockContext.Provider value={props.attributes.accordionName}>
					<InnerBlocks
						allowedBlocks={['tpv/guten-blocks-accordion-element']}
					/>
 ...
 *                             registered; otherwise `undefined`.
 */
registerBlockType('tpv/guten-blocks-accordion-element', {
	// Block name. Block names must be string that contains a namespace prefix. Example: my-plugin/my-custom-block.
...
registerBlockType('tpv/testimonials-carousel', {
	// Block name. Block names must be string that contains a namespace prefix. Example: my-plugin/my-custom-block.
	title: __('Tpv Erfahrungsberichte Karusell'), // Block title.
	icon: 'editor-quote', // Block icon from Dashicons → https://developer.wordpress.org/resource/dashicons/.
	category: 'common', // Block category — Group blocks together based on common traits E.g. common, formatting, layout widgets, embed.
	keywords: [
		__('tpv-guten-blocks — Tpv Erfahrungsberichte Karusell'),
		__('create-guten-block'),
	],
	attributes: {
		carouselItem: {
			...
			source: 'attribute',
			default: parseInt( 100000 * Math.random() ) + '-acc-container',
			selector: 'div.tpv-testimonials-carousel',
			attribute: 'id'
		},
		content: {
			type: 'string',
